...
def configs():
    ...
    tableGRPC = Table(
        show_header=True, header_style='bold magenta', title='gRPC Consumer'
    )
    tableGRPC.add_column(
        'Atributo', justify='center', style='cyan', no_wrap=True
    )
...
def campaigns(
    regex: str = typer.Option(
        None, '--regex', '-r', help='Filtro regex para campanhas.'
    ),
):
    ...
    console = Console()
    tableGRPC = Table(
        show_header=True, header_style='bold magenta', title='Campaigns'
    )
    tableGRPC.add_column(
        'campaign_id', justify='center', style='cyan', no_wrap=True
    )
    ...
    for campaign in filtered_campaigns:
        tableGRPC.add_row(
            campaign.campaign_id, campaign.campaign_name, campaign.last_update
        )
...
def tabulations(
    regex: str = typer.Option(
        None, '--regex', '-r', help='Filtro regex para as tabulações.'
    ),
):
    ...
    console = Console()
    tableGRPC = Table(
        show_header=True, header_style='bold magenta', title='Tabulations'
    )
    tableGRPC.add_column(
        'tabulation_id', justify='center', style='cyan', no_wrap=True
    )
    ...
    tableGRPC.add_column(
        'customer_service_survey_id', justify='center', style='magenta'
    )
    tableGRPC.add_column('last_update', justify='center', style='magenta')
    ...
    if regex:
        ...
        filtered_tabulations = [
            tabulation
            for tabulation in tabulations.tabulations
            if (
                pattern.search(tabulation.tabulation_id)
                or pattern.search(tabulation.tabulation_name)
                or pattern.search(tabulation.tabulation_type)
                or pattern.search(tabulation.group_name)
                or (
                    tabulation.customer_service_survey_id
                    and pattern.search(tabulation.customer_service_survey_id)
                )
                or pattern.search(tabulation.last_update)
            )
        ]
...
def user_state(
    regex: str = typer.Option(
        None, '--regex', '-r', help='Filtro regex para os estados do usuário.'
    ),
):
    ...
    console = Console()
    tableGRPC = Table(
        show_header=True, header_style='bold magenta', title='User States'
    )
    tableGRPC.add_column(
        'user_id', justify='center', style='cyan', no_wrap=True
    )
    ...
    if regex:
        ...
        filtered_user_states = [
            userstate
            for userstate in userstates.user_states
            if (
                pattern.search(userstate.user_id)
                or pattern.search(userstate.menu_id)
                or pattern.search(userstate.route)
                or pattern.search(userstate.obs)
                or pattern.search(userstate.date)
                or pattern.search(str(userstate.direction))
            )
        ]
...
def delete_user_state(
    user_id: str = typer.Argument(..., help='ID do UserState a ser deletado.'),
):
    ...
    try:
        ...
        if success:
            ...
            typer.echo(
                f"Falha ao deletar UserState com ID '{user_id}'.", err=True
            )
